...
// Reject if any single piece of any type can solve the puzzle alone from any safe square
...
const POOL=['Q','Q','R','R','B','B','N','N','P','P'];

function generateEasy4(){
  for(let a=0;a<15000;a++){
    const sqs=shuffle([...Array(25).keys()]);
    const eKeys=sqs.slice(0,4);
    const eTypes=shuffle(POOL).slice(0,4);
    const enemies=eKeys.map((k,i)=>({t:eTypes[i],k}));
    // No Pawns on bottom row (row 4, keys 20-24) — they'd attack off-board
    if(enemies.some(e=>e.t==='P'&&e.k>=20))continue;
    // No Kings (not used in this game)
    ...
    if(countSols(enemies,myTypes,2)===1&&noSubset(enemies,myTypes)&&!anyPieceSolvesAlone(enemies))
      return{enemies,myTypes,tier:'easy'};
  }
  return null;
}

const outFile='/home/claude/puzzles_easy4.json';
const TOTAL=90;
let bank=[];
try{bank=JSON.parse(fs.readFileSync(outFile,'utf8'));}catch{}
const start=bank.length;
process.stderr.write(`Resuming easy from ${start}/${TOTAL}\n`);
const t0=Date.now();
for(let i=start;i<TOTAL;i++){
  process.stderr.write(`Generating ${i+1}/${TOTAL}...\n`);
  const p=generateEasy4();
...
process.stderr.write(`Easy complete: ${bank.length} puzzles\n`);
