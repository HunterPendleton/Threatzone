...
function generateMedium(){
  for(let a=0;a<15000;a++){
    ...
    const eKeys=sqs.slice(0,5);
    const eTypes=shuffle(POOL).slice(0,5);
    ...
    if(countSols(enemies,myTypes,2)===1&&noSubset(enemies,myTypes)&&!anyPieceSolvesAlone(enemies))
      return{enemies,myTypes,tier:'medium'};
  }
  return null;
}

const outFile='/home/claude/puzzles_medium.json';
const TOTAL=90;
let bank=[];
try{bank=JSON.parse(fs.readFileSync(outFile,'utf8'));}catch{}
const start=bank.length;
process.stderr.write(`Resuming medium from ${start}/${TOTAL}\n`);
const t0=Date.now();
for(let i=start;i<TOTAL;i++){
  process.stderr.write(`Generating ${i+1}/${TOTAL}...\n`);
  const p=generateMedium();
...
process.stderr.write(`Medium complete: ${bank.length} puzzles\n`);
